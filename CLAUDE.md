...
| `reps-habitos` | `[{id, name, hint, core, days}]` | Hábitos editables (antes constante en el código). IDs inmutables. `days` ∈ `'all'` o array de días de semana (0=dom..6=sáb); lo añadió la migración 5→6. Editor en Hoy. Reglas: máx 8, entre 2 y 4 core. `CORE` se deriva con `rebuildCore()`, pero `isWon(rec, fecha)` usa `coreDelDia(fecha)`: un día sin core programado es **descanso** (neutral: ni ganado ni perdido; `esDescanso`/`esGanado`). |
