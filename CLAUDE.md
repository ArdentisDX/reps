...
| `reps-finanzas` | `{movs:[{id,tipo,monto,cat,nota,fecha,creado}], presupuesto, metas:[{id,nombre,objetivo,ahorrado,creada}]}` | **Finanzas** (en el menú "Más", sheet `#finWrap`). `tipo` ∈ gasto/ingreso; `cat` ∈ ids de `FIN_CATS[tipo]`. `renderFin` calcula saldo/ingresos/gastos del **mes en curso** (`mesActual()`), barra de `presupuesto` (mensual, 0 = sin límite) vs gastos, lista de movimientos y metas de ahorro con aportes (`fmtDinero`, pesos es-MX). 100% local, aditiva, va en el respaldo. |
