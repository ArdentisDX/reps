...
| Clave | Forma | Notas |
|---|---|---|
| `reps-dias` | `{fecha: {habitoId: bool\|número}}` | Día "ganado" = todos los hábitos `core` HECHOS (`isWon`→`hecho(rec,id)`). Un hábito normal se guarda `true`; uno **con contador** (`meta>0`) guarda su progreso del día como **número** y está HECHO solo al alcanzar la meta (un parcial NO gana el día). Registros vacíos son válidos. Las claves son IDs de `reps-habitos`; por eso los IDs son inmutables (renombrar no rompe el historial). |
| `reps-habitos` | `[{id, name, hint, core, days, planB, emoji, porQue, meta, unidad}]` | `porQue` = motivo opcional del hábito (aditivo, como emoji); se muestra en cursiva 💭 en la tarjeta si el hábito no está hecho. El editor permite reordenar arrastrando el asa ⠿ (el orden del array ES el orden visible). Hábitos editables (antes constante en el código). IDs inmutables. `emoji` = ícono opcional del hábito (aditivo, saneado en `sanearEmoji`, sin migración); se muestra antes del nombre en la tarjeta. Editor en Hoy. `days` ∈ `'all'` o array de días de semana (0=dom..6=sáb); lo añadió la migración 5→6. `planB` = versión mínima opcional (aditivo, saneado en `sanearHabitos`, sin migración); se muestra en la tarjeta si el hábito no está hecho. `meta` = meta numérica opcional (entero 1–999, saneada en `sanearMeta`; 0/ausente = sin contador) y `unidad` = etiqueta ("vasos", "páginas"); un hábito con `meta` muestra "n / meta unidad", el check enseña el número, tap suma 1, − resta, swipe completa/reinicia (`setCount`/`incHabit`). Aditivos, sin migración. Editor en Hoy. `tras` = ancla opcional ("después de X, hago esto"; aditivo, se muestra en cursiva ⛓ en la tarjeta si no está hecho). `subs` = sub-tareas opcionales `[{id, txt}]` (idea #36, máx 10, saneadas en `sanearSubs`): checklist de pasos en la tarjeta (si el hábito no está hecho); su progreso por día vive en `reps-habsub` `{fecha:{habId:{subId:true}}}` (aparte, no toca `reps-dias`); al marcar TODAS, `toggleSub` llama `setHabit(h,true)` y el hábito queda hecho. Editor en Hoy (textarea, una por línea). `frecSemanal` = entero 1–7 opcional (idea #71): el hábito se hace "N× por semana" sin fijar días; un hábito con `frecSemanal` **nunca es core** (`sanearHabitos` fuerza core=false) y en la tarjeta muestra "n / N esta semana" (`frecHechaSemana`, cuenta lun–hoy). Ambos aditivos, saneados (`sanearFrec`), sin migración. Reglas: máx 8, entre 2 y 4 core. `CORE` se deriva con `rebuildCore()`, pero `isWon(rec, fecha)` usa `coreDelDia(fecha)`: un día sin core programado es **descanso** (neutral: ni ganado ni perdido; `esDescanso`/`esGanado`). **Regla clave: toda comprobación de "hecho" pasa por `hecho(rec,id)`** (no por `rec[id]` crudo) para que el contador no rompa isWon/puntaje. |
| `reps-bandeja` | `[{id, text, cat, done, created}]` | Ideas, más nuevas primero. `cat` ∈ ya/social/compras/aprender/algundia. `addIdea(texto, cat)` es la única puerta de entrada. **Bandeja inteligente**: al capturar una idea, `sugerirCategoria(texto)` pide a `/ia` que la clasifique y resalta la categoría sugerida en el selector (`.cp-opt.sug` + "🤖 sugiere: …"); consulta suelta y stateless, sin internet cae a manual. |
| `reps-brief` | `{fecha, texto}` | **Resumen de la mañana**: tarjeta en Hoy (`#brief`) que la IA arma 1×/día con lo que toca hoy (evento, bloque en curso, un consejo). Cacheada por fecha (`generarBrief`/`renderBrief`): se regenera al cambiar de día o con el botón ↻; sirve offline con el último. Transitoria: **NO va en el respaldo** (como `reps-stories`). |
| `reps-finanzas` | `{movs:[{id,tipo,monto,cat,nota,fecha,creado,dir?,meta?,sub?,cart?}], presupuesto, presuCat, metas, subs, deudas, carteras, noGasto}` | **Finanzas** (menú "Más", sheet `#finWrap`). `tipo` ∈ gasto/ingreso/**ahorro**; `cat` ∈ ids de `FIN_CATS[tipo]`. `renderFin` calcula saldo del **mes en curso** (`mesActual()`) = ingresos − gastos − **ahorro apartado** (idea #79: aportar a una meta empuja un mov `tipo:'ahorro',dir:'in'` para que tu saldo real baje; Retirar usa `dir:'out'`; estos movs NO salen en la lista ni en el desglose). `presuCat` = topes por categoría = **sobres** (idea #80, muestra "queda $X"). `subs` = gastos fijos (se auto-registran con `procesarSubs`, dedupe por `m.sub`); **Próximos pagos** (#81) los ordena por cuándo tocan. **Gastos hormiga** (#82): detección LOCAL (sin IA) de ≥5 compras ≤$150/mes. `noGasto` = categorías vigiladas para la racha **Días sin gastar** (#83). **Esta semana** (#78) compara gasto lun–hoy vs semana previa. Coach IA (`#finCoach`) aparte. 100% local, aditiva, va en el respaldo. |
| `reps-tema-auto` | `"1"` o ausente | **Tema automático por hora**: si activo, `selEfectivo()` alterna el tema — de día (7:00–19:00) usa el preset claro, de noche el tema oscuro elegido (si el elegido ES claro, ese de día y Carbón de noche). Se re-evalúa al volver la app al frente. Toggle en Ajustes. Aditiva, va en el respaldo. |
...
| `reps-evitar` | `[{id, nombre, emoji, desde, creado}]` | **Días sin…** (hábitos a evitar). `desde` = fecha "YYYY-MM-DD" del último día limpio; `diasSin(desde)` = días transcurridos. Tarjeta "Sin recaídas" en Hoy (`renderEvitar`) con botón Recaí (reinicia `desde`=hoy). Editor desde la tarjeta y desde el menú Más → "Días sin…". Aditiva, va en el respaldo. |
| `reps-diario` | `{fecha: "texto"}` | **Diario del día**: escritura libre por fecha, en su propia pantalla (`#diarioWrap`, acceso en Mi día). Textarea de hoy (auto-guardado) + historial. Aparte del cierre. Aditiva, va en el respaldo. |
| `reps-sueno` | `{fecha: {acostar, despertar}}` | **Registro de sueño** (pantalla propia `#suenoWrap`, acceso en Mi día). Horas "H:MM"; `minDormidos` maneja el cruce de medianoche; muestra horas de hoy, promedio y últimas noches. Aditiva, va en el respaldo. |
...
| `reps-retos` | `[{id, texto, meta, hechos, limite, creado}]` | **Retos** (idea #75): objetivo con meta numérica, avance manual (+1/−) y `limite` "YYYY-MM-DD" opcional. Pantalla propia `#retosWrap` (menú Más). `diasRestantes(limite)` da los días que faltan/vencidos; barra = hechos/meta. Aditiva, va en el respaldo. |
| `reps-energia` | `{fecha: 'verde'\|'ambar'\|'rojo'}` | **Semáforo de energía** (idea #77): tarjeta en Hoy (`#energiaCard`) donde marcas tu energía del día; `sugEnergia` sugiere cómo encarar los core pendientes (con todo / a ritmo / bajar al plan B). Segundo toque quita. Aditiva, va en el respaldo. |
| `reps-solo` | `"habitoId"` o ausente | **Modo un solo hábito** (idea #74): filtro VISUAL que en Hoy muestra solo ese hábito (banner `#soloBanner` para salir); no toca puntaje/racha. Si el hábito no existe o no toca hoy, se ignora (muestra todo). Se activa con 🎯 en el editor. Aditiva, va en el respaldo. |
| `reps-eventos` | `[{id, nombre, fecha}]` | **Cuenta regresiva** (idea #115): fechas importantes; `proximoEvento()` (el futuro más cercano) muestra `#cuentaCard` en Hoy con los días que faltan. Sheet `#eventosWrap` (menú Más). Aditiva, va en el respaldo. |
| `reps-viaje` | `{activo, dias:{fecha:true}}` | **Modo viaje/vacaciones** (idea #116): mientras `activo`, cada día que abres la app se marca en `dias`; `coreDelDia` devuelve `[]` para esos días → son **descanso** (neutral, la racha se sostiene). Al terminar, HOY vuelve a contar (`setViaje(false)` borra `dias[today()]`) pero los días PASADOS de viaje quedan neutrales (historia real). `loadViaje` sana un "hoy" atascado: si el viaje NO está activo, quita hoy de `dias`. Banner `#viajeBanner` en Hoy, toggle en menú Más. `viaje` se declara al inicio del IIFE (coreDelDia lo consulta desde el arranque). Aditiva, va en el respaldo. |
| `reps-rutina-tpl` | `[{id, nombre, bloques}]` | **Plantillas de rutina propias** (idea #114): guarda tu rutina actual con nombre (ej. «Entre semana», «Finde») y aplícala; conviven con las 4 plantillas base fijas (`RUTINA_PLANTILLAS`). Editor en «Editar rutina» → «Empezar desde una plantilla». Aditiva, va en el respaldo. |
| `reps-mantra` | `"texto"` o ausente | **Mantra** (idea #97): frase fija arriba de Hoy (`#mantra`); tap para editar con `prompt`. Aditiva, va en el respaldo. |
| `reps-carta-futuro` | `{texto, escrita, abre}` o ausente | **Carta a tu futuro yo** (idea #96): se sella al escribirla (`abre` = 1 año después de `escrita`); `renderCartaFut` muestra editor / sellada (cuenta regresiva) / abierta según la fecha. Sheet `#cartaFuturoWrap` (menú Más). Aditiva, va en el respaldo. |
| `reps-victorias` | `[{id, texto, fecha}]` | **Muro de victorias** (idea #98): momentos de orgullo, más nuevos primero. Sheet `#victoriasWrap` (menú Más). Aditiva, va en el respaldo. |
| `reps-brutal` | `"1"` o ausente | **Modo espejo brutal** (idea #100): toggle en El Espejo (Stats) que muestra `#brutalCard` con números duros (días con core, ganados/perdidos, tasa real, racha vs mejor). Aditiva, va en el respaldo. Las frases de identidad #99 son derivadas (sin clave). |
| `reps-peso` | `{fecha: kg}` | **Peso / medidas** (idea #43, hub Bienestar): peso por día; `pesoChart()` dibuja la tendencia (últimos 30) en un canvas leyendo las variables del tema con `getComputedStyle` (mismo patrón que "Compartir mi mes"). Sheet `#pesoWrap`. Aditiva, va en el respaldo. |
| `reps-gratitud` | `{fecha:[s1,s2,s3]}` | **Gratitud** (idea #93, hub Bienestar): 3 cosas buenas del día + historial + "hace un año" (misma fecha del año anterior). Aditiva, va en el respaldo. |
| `reps-agua` | `{meta, dias:{fecha:count}}` | **Agua** (idea #92, hub Bienestar): vasos del día vs `meta`, con aviso según la hora. Aditiva, va en el respaldo. |
| `reps-pausas` | `{intervalo, activo}` | **Pausas activas** (idea #94, hub Bienestar): mientras la app está abierta y `activo`, un `setInterval` cada `intervalo` min lanza un toast con un estiramiento. Aditiva, va en el respaldo. |
| `reps-sueno-rutina` | `{fecha:{pasoId:true}}` | **Higiene del sueño** (idea #95, hub Bienestar): checklist de bajada nocturna (`SR_PASOS`), estado por noche. Aditiva, va en el respaldo. |
| `reps-checkin` | `{fecha:{mid}}` | **Chequeo de ánimo a media jornada** (idea #91): tarjeta en Hoy (`#checkinCard`) visible 11–17h si aún no lo hiciste; 1 tap (😊😐😔). Aditiva, va en el respaldo. |
| `reps-ia-chat` | `[{who:'yo'\|'ia', text}]` | **Memoria del asistente** (idea #84): hilo completo del chat con la IA, guardado (cap 60 msgs) para que RECUERDE entre sesiones. El Worker `/ia` es stateless: cada turno le mandamos `contextoIA()` + los últimos ~16 mensajes. Sheet `#iaWrap` (chat `#iaChat`), botón "Olvidar" borra. Aparte: **Desatóralo** (#86, `#desatoraWrap`) da UN primer paso; **Reencuadre** (#87, `#reencuadreWrap`) mini-TCC; ambos one-shot con fallback offline. Va en el respaldo. |
